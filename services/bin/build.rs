...
fn main() {
    ...
    sails_idl_gen::generate_idl_to_file::<awesome_sails_services::test::TestProgram>(&idl_path)
        .unwrap();

    // Generate client code from IDL file
    ClientGenerator::from_idl_path(&idl_path)
        .generate_to(PathBuf::from(env::var("OUT_DIR").unwrap()).join("awesome_client.rs"))
        .unwrap();
}
